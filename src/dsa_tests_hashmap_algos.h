#ifndef __hashmap_algos_tests_h__
#define __hashmap_algos_tests_h__

#include "dsa_darray.h"
#include "dsa_hashmap.h"
#include "dsa_hashmap_algos.h"
...
int gen_keys(DArray *, int);
void destroy_keys(DArray *);
void fill_distribution(int *, DArray *, Hashmap_hash);
...
void destroy_keys(DArray *keys) {
  int i = 0;
  for (i = 0; i < NUM_KEYS; i++) {
    bdestroy((bstring)DArray_get(keys, i));
  }
